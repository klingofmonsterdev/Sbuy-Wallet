...
import { alertOops } from './constants/Function';
                ...
                <PINCode
                    titleComponent={() => {
                        return (
                            <View style={{ backgroundColor: '#58A3EA', width: width, height: height * 0.3 }}>
                                ...
                                <Text style={{ fontSize: 16, color: "#FFFFFF", alignSelf: 'center' }}>Enter your PIN - 4 digits</Text>
                            </View>
                        )
                    }
                    }
                    ...
                    handleResultEnterPin={(result) => {
                        ...
                        AsyncStorage.getItem('passcode')
                            .then((res) => {
                                let resultRes = JSON.parse(res);
                                if (resultRes.passcode == result) {
                                    Actions.Home();
                                    // alertOops("Opps..!", "Passcode ถูกต้อง..!")
                                } else {
                                    alertOops("Opps..!", "Passcode ไม่ถูกต้อง กรุณาลองใหม่อีกครั้ง..!")
                                }
                            })
                            .catch((error) => {
                                console.log(error);
